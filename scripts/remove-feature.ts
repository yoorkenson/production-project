...
// для поиска названий функций для работы с node, чекать AST (абстрактно синт дерево)

const removerFeatureName = process.argv[2]; // example isArticleEnabled
const featureState = process.argv[3]; // example on/off

const toggleFunctionName = 'toggleFeatures';
const toggleComponentName = 'ToggleFeatures';

if (!removerFeatureName) {
...
const replaceToggleFunction = (node: Node) => {
    ...
    if (featureName !== removerFeatureName) return;
